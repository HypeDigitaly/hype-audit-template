...
<head>
  ...
  <style>
    ...
    .header { border-bottom: 2px solid ${clientConfig.brand.primaryColor}; padding-bottom: 10px; margin-bottom: 20px; }
    .label { color: #666; font-size: 12px; text-transform: uppercase; font-weight: bold; }
    .value { font-size: 16px; margin-bottom: 15px; }
    .btn { display: inline-block; padding: 12px 24px; background: ${clientConfig.brand.primaryColor}; color: white; text-decoration: none; border-radius: 4px; font-weight: bold; }
  </style>
</head>
<body>
  <div class="container">
...
<head>
  ...
  <style>
    ...
    .container { max-width: 600px; margin: 0 auto; }
    .btn { display: inline-block; padding: 14px 28px; background: ${clientConfig.brand.primaryColor}; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; margin: 20px 0; }
  ...
  <div class="container">
    ...
    <div class="footer">
      <p style="font-size: 12px; color: #999; margin-bottom: 16px;">
        ...
        <a href="mailto:${clientConfig.primaryContact.email}" style="color: ${clientConfig.brand.primaryColor};">${clientConfig.primaryContact.email}</a> | ${clientConfig.primaryContact.phone}<br>
        <a href="${clientConfig.primaryContact.calendarUrl || ''}" style="color: ${clientConfig.brand.primaryColor};">📅 Book a meeting in my calendar →</a>
      </p>

      ${clientConfig.social?.googleReviews ? `<p style="margin-bottom: 12px;">
        <a href="${clientConfig.social.googleReviews}" target="_blank" style="color: ${clientConfig.brand.primaryColor}; text-decoration: none;">⭐ See our Google Reviews</a>
      </p>` : ''}
      ...
      ${clientConfig.company.legalName} | <a href="${clientConfig.siteUrl}" style="color: ${clientConfig.brand.primaryColor};">${clientConfig.siteUrl.replace('https://', '')}</a><br>
      Report ID: ${reportId}
    </div>
  </div>
</body>
...
<body>
  <div class="container">
    ...
    <p>Doporučení je mnoho a pokud nevíte, kde začít, či byste chtěli jen prokonzultovat, rádi se s Vámi potkáme na osobní schůzce klidně u Vás na firmě.</p>
    ...
    <div class="footer">
      <p style="font-size: 12px; color: #999; margin-bottom: 16px;">
        ...
        ${clientConfig.primaryContact.name} | ${clientConfig.primaryContact.title}<br>
        <a href="mailto:${clientConfig.primaryContact.email}" style="color: ${clientConfig.brand.primaryColor};">${clientConfig.primaryContact.email}</a> | ${clientConfig.primaryContact.phone}<br>
        <a href="${clientConfig.primaryContact.calendarUrl || ''}" style="color: ${clientConfig.brand.primaryColor};">📅 Domluvit schůzku v mém kalendáři →</a>
      </p>

      ${clientConfig.social?.googleReviews ? `<p style="margin-bottom: 12px;">
        <a href="${clientConfig.social.googleReviews}" target="_blank" style="color: ${clientConfig.brand.primaryColor}; text-decoration: none;">⭐ Podívejte se na naše Google recenze</a>
      </p>` : ''}
